...
CAR=2
CTORSION=2
CSIMULATION_DIR = data/phase_model/fixed_filament/
CSIMULATION_NAME = test_bab_$(CNFIL)fil_$(CNBLOB)blob_$(CAR)R_2torsion
...
cilia_nvidia4_withCUFCM: $(CILIA_CPP) $(CILIA_CUDA)
	nvcc $^ -DUSE_DOUBLE_PRECISION $(NVCC_FLAGS) $(NVIDIA4_OPTS) $(LINK) $(GEN_FLAGS) $(CFLAGS) -o bin/$(CSIMULATION_NAME)